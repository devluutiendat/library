...
import { query, getAuthorQuery, getBookQuery } from '../src/query';
...
export default function List() {
  ...
  }, [userLogin, choice, navigate]); // Ensure it fetches data when userLogin or choice changes
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-6">
      <h1 className="text-3xl font-bold text-center mb-8">Data List</h1>
      <div className="flex justify-center mb-6">
        <button
          className={`mx-2 px-4 py-2 rounded ${choice ? 'bg-blue-600 text-white' : 'bg-gray-300'}`}
        ...
        {data
          ...
          .map(item => (
            ...
            className="bg-white border border-gray-200 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-200 p-6">
              {choice ? (
                <>
                  <h2 className="text-xl font-semibold text-blue-600 hover:text-blue-800 transition-colors duration-200">{item.title}</h2>
                  <p className="text-gray-700"><strong>Published Date:</strong> {item.publishedDate}</p>
                  <p className="text-gray-700"><strong>Genre:</strong> {item.genre}</p>
                  <p className="text-gray-700"><strong>Pages:</strong> {item.pages}</p>
                  <p className="text-gray-700"><strong>Author ID:</strong> 
                  {item.authorId === null ? "no updated data" : item.authorId}</p>
                </>
              ) : (
                <>
                  <h2 className="text-xl font-semibold text-blue-600 hover:text-blue-800 transition-colors duration-200">{item.name}</h2>
                  <p className="text-gray-700"><strong>Birthday:</strong> {item.birthday}</p>
                  <p className="text-gray-700"><strong>Nationality:</strong> {item.nationality}</p>
                  <p className="text-gray-700"><strong>Books Count:</strong> {item.books.length}</p>
                </>
              )}
            </li>
          ))}
